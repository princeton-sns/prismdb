...
#default_log_path = '/home/user/hetsys/prismdb/p2/scripts/logs'
...
def create_config_str(cfg):
    ...
    exp_str = code+"_"+ssd+"_"+num_partitions+"_"+bc_size+"_"+dist+"_"+dp+"_"+pc_size+"_"+pc_cache_thresh+"_"+db_size+"_"+key_size+"_"+value_size+"_"+operations+"_"+num_clients+"_"+workload+"_"+migration_policy+"_"+migration_rand_range_num
    return exp_str
...
def run_single_exp(exp_cfg, log_folder):
    ...
    ycsb_dict = {'ycsba':'ycsbwklda', 'ycsbb':'ycsbwkldb', 'ycsbc':'ycsbwkldc', 'ycsbd':'ycsbwkldd','ycsbe':'ycsbwklde','ycsbf':'ycsbwkldf'}
    loop = 0
    ...
    print ("COMMAND: sudo cgexec -g memory:mlsm ./db_bench", arg_str)
    ...
    subprocess.call(["( cd "+prismdb_src_path+"/build; sudo cgexec -g memory:mlsm ./db_bench "+arg_str+")"], shell=True)
    io.kill()
    cpu.kill()
  
    # kill background iostat if not killed successfully
    subprocess.call(["( var=$(ps ax | grep iostat | head -1 | awk '{print $1}'); kill $var )"], shell=True)
    

    # move log files
    #subprocess.call(["mv /home/user/hetsys/"+ssd+"/db_ash/LOG "+log_folder], shell=True);
