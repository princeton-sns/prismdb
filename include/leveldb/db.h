...
class LEVELDB_EXPORT DB {
 public:
  ...
  virtual Status Get(const ReadOptions& options, const Slice& key,
                     std::string* value) = 0;
  // Scan() returns the next scan_size entries starting from
  // start_key in the database if found,
  // stores them as pairs of key and value in the vector - results
  // and set the return status as OK.
  //
  // May return some other Status on an error.
  virtual Status Scan(const ReadOptions& options, const Slice& start_key,
                      const uint64_t scan_size,
                      std::vector<std::pair<Slice, Slice>>* results) = 0;
  ...
  virtual void SetPopFile(const char* pop_file) = 0;
};
